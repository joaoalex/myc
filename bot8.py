...
getRealData=True
try:
     if sys.argv[2]=="FILE" and (runmode=="SIMUL"  or runmode=="CHART"):
        getRealData=False
except:
     getRealData=True
...
datarunned=False
millseconds_last=0
first_millseconds_next=1677628800 #1-2-2023
#first_millseconds_next=1685750400
...
curasset.path=''
if runmode=='SIMUL'  or runmode=="CHART":
...
while True:
         ...
         if loopCFG:
            ...
            for cfg in symbol_cfg:
                     ...
                     for i in range(9):
                         ...
                         if runmode=="REAL":
                            ...
                            for line in klines_aux: 
                               ...
                               if runmode=="SIMUL" or runmode=="CHART":
                                if round(float(line[0])/1000,0) >= millseconds_last and round(float(line[0])/1000,0) < millseconds_next:
                                  #if i==5:
                                  #   sys.stdout.flush()
                                  #   sleep(20)
                                  data.append([datetime.fromtimestamp(float(line[0])/1000), line[1], line[2], line[3], line[4],line[5]])
                                ...
                                if round(float(line[0])/1000,0) >= millseconds_next:
                                  break
               ...
               if True:   #indicators 
                     ...
                     upper1w, middle1w, lower1w = talib.BBANDS(df1w.close,timeperiod=14, nbdevup=2, nbdevdn=2)
                     curasset.middle1w  = round(float(middle1w[len(middle1w)-1]),curasset.decplaces)
               ...
               if accountData and runmode=="REAL":
                     ...
                     try:
                        ...
                        curasset.lastsafeuse=float(linecfg[3])
                         ...
                         print('ERROR ON READING SAFE FILE')
                         curasset.lastposition=round(curasset.atualasset,2)
                         curasset.safe=0
                         curasset.maxsafe=0              
                         curasset.lastsafeuse=0              
                     
               if accountData and (runmode=="SIMUL"  or runmode=="CHART"):
               ...
               if checkOrders and runmode=="SIMUL" or runmode=="CHART":
                  a=[]
               ...
               if runmode=="CHART":
                 millseconds_next=millseconds_next + 86400

               else:
                 millseconds_next=millseconds_next + 14400 #3600 #3600 #14400 # 15m 900
      ...
      if True and runmode=="SIMUL" or runmode=="CHART":
         #   print('----------- ',balance,' ------------')
             ...
             if True:
               ...
               print(df.index[len(df.close)-1], ';',numpy.format_float_positional(round(float(price['price']), curasset.decplaces)).rjust(10,' '), ';',curasset.rsi7h, ';',curasset.rsi1h, ';', curasset.macd1hlast, ';', curasset.macd4hlast, ';', curasset.macd12hlast, ';', curasset.macd1dlast, ';',curasset.macd3dlast, ';',curasset.macd1wlast, ';', change, '% ;', round(curasset.margin,2), ';',round(curasset.levdefault,2), ';',  round(float(curasset.netasset),5) , ';P;',round(float(curasset.atualasset),0), ';', round(float(curasset.safe) , 0),  ';', round(curasset.regression,0), ';', round(curasset.regressionDay,0), ';', curasset.ema1w7, ';',curasset.ema1w14, ';',  curasset.ema1d100,';', curasset.ema1d200,';', curasset.ema1d14, ';',curasset.middle1w,';', curasset.path) 
               if dfresult.empty:
                  # initialize list of lists
                  data = [[df.index[len(df.close)-1], round(float(price['price']), curasset.decplaces),  curasset.middle1w, curasset.ema1w7,curasset.ema1w14,curasset.ema1d100, curasset.ema1d200 ,curasset.regressionDay ]]
                  print(data)
                  dfresult =   pd.DataFrame(data, columns=['data', 'price', 'middle1w','ema1w7', 'ema1w14', 'ema1d100','ema1d200', 'regression'])
               
               else:
                
                  dfresult.loc[len(dfresult.index)] = [df.index[len(df.close)-1],  round(float(price['price']),curasset.decplaces), curasset.middle1w, curasset.ema1w7 , curasset.ema1w14, curasset.ema1d100,curasset.ema1d200, curasset.regressionDay]
             #, curasset.path )  
      ...
      if ( ( runmode == 'SIMUL' or runmode == 'CHART' ) and progress==0 ):
         #print(round(curasset.atualasset,2))
         with open('do.log', 'a') as z:
            #print('B1',B1,'B2', B2, 'B3', B3,'C1',C1,'C2',C2, 'C3',C3,'D1',D1,'D2', D2, file=z)
            print('Progress', round(progress/totalrec*100,0), round(float(curasset.atualasset),0), round(float(curasset.safe) , 0), df.index[len(df.close)-1], curasset.margin, curasset.levcritical, curasset.levmin, curasset.levdefault, curasset.levmax, file=z)
         z.close()
         print('END')
         #df.plot(kind='line',x='df.date',y='close')
         #plt.plot(df.index, df.close)
         print(dfresult)
         #dfresult.plot(x=, dfresult.price, dfresult.middle1w)
         fig,ax=plt.subplots()

         ax.plot(dfresult.data, dfresult.price) #, marker="o")
         ax.set_xlabel("data")
         ax.set_ylabel("price")
         #ax2 = ax.twinx()

         #plt.plot(dfresult.data, dfresult.price, dfresult.middle1w)  
         ax.plot(dfresult.data, dfresult.middle1w) #, marker="o")
         ax.plot(dfresult.data, dfresult.ema1w7) #, marker="o")
         ax.plot(dfresult.data, dfresult.ema1w14) #, marker="o")
         ax.plot(dfresult.data, dfresult.ema1d100) #, marker="o")
         ax.plot(dfresult.data, dfresult.ema1d200) #, marker="o")
         ax.legend(['price', 'middle1w','ema1w7', 'ema1w14', 'ema1d100', 'ema1d200'])

         #ax2.plot(dfresult.data, dfresult.regression) #, marker="o")
         #ax2.set_xlabel('regression',color="#11aa00")
         #ax2.legend(['regression']),
